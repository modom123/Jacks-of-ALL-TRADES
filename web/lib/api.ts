...
export const api = {
  ...
  getProducts: () =>
    apiFetch<{ products: import('./supabase').Product[] }>('/api/shop/products'),
  ...
  admin: {
    ...
    getApplications: (
      ...
      return apiFetch<{
        applications: import('./supabase').Application[];
        total: number;
        page: number;
        limit: number;
      }>(`/api/admin/applications?${qs.toString()}`, { token });
    },
    ...
    getContacts: (token: string, params: { page?: number } = {}) => {
      const qs = new URLSearchParams();
      if (params.page) qs.set('page', String(params.page));
      return apiFetch(`/api/admin/contacts?${qs.toString()}`, { token });
    },

    getDonations: (token: string, params: { page?: number } = {}) => {
      const qs = new URLSearchParams();
      if (params.page) qs.set('page', String(params.page));
      return apiFetch(`/api/admin/donations?${qs.toString()}`, { token });
    },

    getVolunteers: (token: string, params: { page?: number } = {}) => {
      const qs = new URLSearchParams();
      if (params.page) qs.set('page', String(params.page));
      return apiFetch(`/api/admin/volunteers?${qs.toString()}`, { token });
    },
  },
};
